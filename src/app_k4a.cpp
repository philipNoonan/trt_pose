...
void App::init(k4a_depth_mode_t depthMode, k4a_color_resolution_t colorResolution) {
	// Check for devices
	//
...
bool App::openFile(std::string filename) {

	fileHandle = k4a::playback::open(filename.c_str());

	calibration = fileHandle.get_calibration();
	auto configRecord = fileHandle.get_record_configuration();
	fileHandle.set_color_conversion(K4A_IMAGE_FORMAT_COLOR_BGRA32);
	auto fileLength = fileHandle.get_recording_length();
	...
	if (fileHandle.get_next_capture(&cap)) {
		...
		startTime = depthImage.get_device_timestamp();
	}


	//fileHandle.set_color_conversion()

   // while (handle.get_next_capture(&cap))
	//{
	   // depthImage = cap.get_depth_image();
	   // auto time = depthImage.get_device_timestamp();

		//do sth here with the capture..
   // }

	return true;
}

bool App::getImagesFromFile(k4a::image& depthImage, k4a::image& colorImage, k4a::image& infraImage) {

	if (positionChanged) {
		fileHandle.seek_timestamp(std::chrono::microseconds(position), K4A_PLAYBACK_SEEK_BEGIN);
		positionChanged = false;

		if (fileHandle.get_next_capture(&cap)) {
			depthImage = cap.get_depth_image();
			colorImage = cap.get_color_image();
			infraImage = cap.get_ir_image();
...
bool App::getImages(k4a::image& depthImage, k4a::image& colorImage, k4a::image& infraImage) {
...
bool App::getIMU(k4a_imu_sample_t& imuSample) {
...
void App::mainLoop()
{
	...
	if (commandLineParser.isSet("input")) {
		k4a_mkv_filename_ = commandLineParser.getValueAsString("input", "");
	}
	if (commandLineParser.isSet("output")) {
		size_t pos = k4a_mkv_filename_.find(".mkv");
		std::string outputdir;
		if (pos != std::string::npos) {
			outputdir = k4a_mkv_filename_.substr(0, pos);
		}
	...
	frameReady = getImagesFromFile(depthImage, colorImage, infraImage);

	std::cout << colorImage.get_width_pixels() << " " << colorImage.get_height_pixels() << std::endl;

	frameReady = getImagesFromFile(depthImage, colorImage, infraImage);

	std::cout << colorImage.get_width_pixels() << " " << colorImage.get_height_pixels() << std::endl;

	depth_width_ = depthImage.get_width_pixels();
	depth_height_ = depthImage.get_height_pixels();

	image_width_ = colorImage.get_width_pixels();
	image_height_ = colorImage.get_height_pixels();

	std::cout << colorImage.get_size() << std::endl;

	depthInColorSaceImage.create(K4A_IMAGE_FORMAT_DEPTH16,
		image_width_,
		image_height_,
		image_width_* (int)sizeof(uint16_t));

	auto trans = k4a::transformation(calibration);

	//depthInColorSaceImage = trans.depth_image_to_color_camera(depthImage);

	//k4a::transformation::depth_image_to_color_camera(depthImage, &depthInColorSaceImage);
	...
	depth_frame_->createStorage(1, depth_width_, depth_height_, GL_RED_INTEGER, GL_R16UI, GL_UNSIGNED_SHORT, false);
	...
	while (!glfwWindowShouldClose(window_)) {
		...
		depthInColorSaceImage = trans.depth_image_to_color_camera(depthImage);
		xyzImage = trans.depth_image_to_point_cloud(depthInColorSaceImage, K4A_CALIBRATION_TYPE_COLOR);
		...
		color_frame_->update(colorImage.get_buffer());
		depth_frame_->update(depthImage.get_buffer());
		...
		for (int i = 0; i < mNumObjects; i++)
		{
			...
			for (int i = 0; i < obj_pose.Keypoints.size(); i++) {
				float pixX = (obj_pose.Keypoints[i].x / 255.0f) * float(image_width_);
				float pixY = (obj_pose.Keypoints[i].y / 255.0f) * float(image_height_);

				//float depth = aligned_depth_frame.get_distance(obj_pose.Keypoints[i].x / 255.0f * 848.0f, obj_pose.Keypoints[i].y / 255.0f * 480.0f);
				int index = pixY * float(image_width_) + pixX;
				uint16_t *depth_ptr = (uint16_t*)(void*)depthInColorSaceImage.get_buffer();
				float depth = depth_ptr[index];
				k4a_float2_t sourcePoint{pixX, pixY};

				k4a_float3_t targetPoint{};

				calibration.convert_2d_to_3d(sourcePoint, depth, k4a_calibration_type_t::K4A_CALIBRATION_TYPE_COLOR, k4a_calibration_type_t::K4A_CALIBRATION_TYPE_COLOR, &targetPoint);
				...
				detected_keypoints.push_back(targetPoint.xyz.x);
				detected_keypoints.push_back(targetPoint.xyz.y);
				...
				detected_keypoints_render.push_back(targetPoint.xyz.z);
			}

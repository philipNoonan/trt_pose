...
namespace gl
{
	...
	public:
		...
		void clear(GLuint internalFormat, GLuint format, GLuint type, int offset, int size, const void * data)
		{
			glBindBuffer(BufferType, id);
			glClearBufferSubData(BufferType, internalFormat, offset, size * sizeof(DataType), format, type, data);
			glBindBuffer(BufferType, 0);
		}
